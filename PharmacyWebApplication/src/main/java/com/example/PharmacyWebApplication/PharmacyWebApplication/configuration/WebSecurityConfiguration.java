    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception {
        ...
        httpSecurity
                .csrf(customizer -> customizer
                        .ignoringRequestMatchers("/authenticate","/registerNewUser","/product/list", "/product/add", "product/list/{id}", "product/update/{id}", "product/delete/{id}", "/category/create", "/category/list", "/category/update/{categoryId}", "/category/delete/{categoryId}","/placeOrder", "/addToCart/{id}", "/getCartDetails", "/getProductDetails/{isSingleProductCheckout}/{id}", "/updateCartItemQuantity/{cartId}", "/deleteCartItem/{cartId}", "/placeOrder/{isSingleProductCheckout}", "/getAllOrderDetails/{status}", "/api/v1/files", "/api/v1/prescriptions", "/api/v1/prescriptions/add", "/api/v1/prescriptions/list", "/api/v1/prescriptions/getPrescriptionDetails")) // Exclude "/authenticate" from CSRF protection
                .authorizeRequests()
                .requestMatchers("/authenticate","/registerNewUser","/product/list", "/product/add", "product/list/{id}", "product/update/{id}", "product/delete/{id}", "/category/create", "/category/list", "/category/update/{categoryId}", "/category/delete/{categoryId}", "/placeOrder", "/addToCart/{id}", "/getCartDetails", "/getProductDetails/{isSingleProductCheckout}/{id}", "/updateCartItemQuantity/{cartId}", "/deleteCartItem/{cartId}", "/placeOrder/{isSingleProductCheckout}", "/getAllOrderDetails/{status}", "/api/v1/files", "/api/v1/prescriptions" , "/api/v1/prescriptions/add", "/api/v1/prescriptions/list", "/api/v1/prescriptions/getPrescriptionDetails").permitAll()
